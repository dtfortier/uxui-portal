...
function bootstrapcanvaswp_setup() {
	...
	register_nav_menus( array(
        ...
        'mockups-sub-menu' => __('Mockups Sub Menu'),
        'prototypes-sub-menu' =>__('Prototypes Sub Menu'),
	) );
...
function my_get_posts( $query ) {
	if ( is_home() && $query->is_main_query() || is_feed())
		$query->set( 'post_type', array( 'post', 'page', 'mockup', 'prototype', 'standard' ) );
	return $query;
}
...
'post_type' => array('post',  'mockup', 'prototype', 'standard')
);

//Loop to display 8 recently updated posts
$lastupdated_loop = new WP_Query( $lastupdated_args );
$counter = 1;
$string .= '<ul>';
while( $lastupdated_loop->have_posts() && $counter < 8 ) : $lastupdated_loop->the_post();
$string .= '<li><a href="' . get_permalink( $lastupdated_loop->post->ID ) . '"> ' .get_the_title( $lastupdated_loop->post->ID ) . '</a>  <div class="author">' . get_the_author() .' added a ' . get_post_type() . ' </div> <div class="date">'. get_the_modified_date() .'</div> ' . get_the_category_list($parents, $post_id ) .'  </li>';
$counter++;
endwhile;    
$string .= '</ul>';
return $string;
